...
def is_centered(self) -> bool:
    """
    Check whether the entity is close enough to the centre of its
    current tile.

    Computes the nearest tile centre from the entity's pixel position
    and returns True if the distance in both axes is within a tolerance
    equal to the entity's current speed. This is used to gate direction
    changes so entities only turn at tile boundaries, preventing clipping
    into walls.
    ...
    Returns:
        bool: True if the entity is within one speed-unit of the tile
        centre on both axes, False otherwise.
    """
...
def check_collision(self, direction) -> bool:
    ...
    next_rect = pygame.Rect(
        next_x, next_y, self.rect.width, self.rect.height
    )
...
def reset_position(self) -> None:
    """
    Teleport the entity back to its spawn position and clear its movement
    state.
    ...
    direction and the queued next direction. Called after the player loses
    a life or when starting a new round so all entities begin from their
